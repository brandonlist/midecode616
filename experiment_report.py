...
def fit_AAR_module(log_name,datasets,model,ch,out_file,**kwargs):
    ...
    with open(log_name, 'w') as f:
        ...
        out_file.write('对' + str(datasets.n_subject) + '名被试做平均，'+'参数'+str(paras_AAR_sub[int(max_arg_para)]).rstrip('}').lstrip('{').replace(':','=').replace('\'','')+'下模型的表现最好，准确率达到'+str('{:.2f}').format(max_mean_para+ch)+'。')
        ...
        out_file.write('对所有参数做平均，被试'+str(max_arg_sub+1)+'的数据集训练得到的模型表现最好，模型的平均准确率为'+str('{:.2f}').format(max_mean_sub+ch)+'。')
        out_file.write('\n')
...
def fit_AAR_module_const(log_name,datasets,model,ch,out_file,subjects=None,**kwargs):
    ch_dict = {'AAR_LDA':'使用线性判别分析的自回归模型','AAR_SVC':'使用支持向量机的自回归模型','AAR_KNN':'使用k最近邻分类器的自回归模型'}
    if subjects == None:
        subjects = range(int(datasets.n_subject))
    subject_datasets = []
    in_chan = datasets.signal_shape()[0]
    time_steps = datasets.signal_shape()[1]
    classes = datasets.n_classes
        ...
        for j, exp_dataset in enumerate(subject_datasets):
            if j in subjects:
                ...
                if model == 'AAR_LDA':
                    ...
                    clf_0 = AAR_estimator_LDA(in_chan=in_chan, classes=classes,fs=datasets.fs,maxlag=maxlag_lda)
                    clf_0.fit(x_train.numpy(), y_train.numpy())
                    joblib.dump(clf_0, 'subject'+str(j)+'_'+'best_AAR_estimator_LDA.m')
                    # test
                    x_test, y_test = next(iter(DataLoader(test_data, len(test_data))))
                    score = clf_0.score(x_test.numpy(), y_test.numpy())
                ...
                if model == 'AAR_SVC':
                    ...
                    f.write('estimate AAR_SVC for subject' + str(j) + '\n')
                    # train and module selection
                    clf_1 = AAR_estimator_SVC(in_chan=in_chan, classes=classes, fs=datasets.fs, maxlag=maxlag_svc)
                    clf_1.fit(x_train.numpy(), y_train.numpy())
                    joblib.dump(clf_1, 'subject'+str(j)+'_'+'best_AAR_estimator_SVC.m')
                ...
                if model == 'AAR_KNN':
                    ...
                    clf_2 = AAR_estimator_KNN(in_chan=in_chan, classes=classes, fs=datasets.fs, maxlag=maxlag_knn,k=k)
                    clf_2.fit(x_train.numpy(), y_train.numpy())
                    joblib.dump(clf_2, 'subject'+str(j)+'_'+'best_AAR_estimator_KNN.m')
...
def fit_CSP_module(log_name,datasets,model,ch,out_file,**kwargs):
...
def fit_CSP_module_ar(log_name,datasets,model,ch,out_file,**kwargs):
    ch_dict = {'CSP_SVC': '共空间模式模型', 'CSSP_LDA': '共空间频谱模式', 'CSSSP_LDA': '共稀疏频谱空间模式', 'SBCSP_LDA': '子带共空间模式',
               'FBCSP_LDA': '滤波器组共空间模式', 'DFBCSP_FR_LDA': '基于费雪比的判别滤波器组共空间模式'}
    ...
    with open(log_name, 'w') as f:
        ...
        for j, exp_dataset in enumerate(subject_datasets):
            ...
            if model == 'FBCSP_LDA':
                ...
                f.write('select_ratio' + str(select_ratio_fbcsp) + '\n')
                gs = GridSearchCV(FBCSP_estimator_LDA(fs=fs,csp_cpn=3), param_grid=params, cv=4)
                gs.fit(x_train.numpy(), y_train.numpy())
                f.write(str(gs.grid_scores_) + '\n')
                clf_4 = gs.best_estimator_
                joblib.dump(clf_4,'subject'+str(j)+'_'+ 'best_FBCSP_estimator_LDA.m')
            ...
            if model == 'DFBCSP_FR_LDA':
                ...
                gs = GridSearchCV(DFBCSP_FR_estimator_LDA(C3_idx=0, C4_idx=1, fs=fs,csp_cpn=3), param_grid=params, cv=4)
                gs.fit(x_train.numpy(), y_train.numpy())
                f.write(str(gs.grid_scores_) + '\n')
                clf_5 = gs.best_estimator_
                joblib.dump(clf_5,'subject'+str(j)+'_'+'best_DFBCSP_FR_estimator_LDA.m')
...
def fit_CSP_module_ar_2(log_name,datasets,model,ch,out_file,**kwargs):
    ch_dict = {'CSP_SVC': '共空间模式模型', 'CSSP_LDA': '共空间频谱模式', 'CSSSP_LDA': '共稀疏频谱空间模式', 'SBCSP_LDA': '子带共空间模式',
               'FBCSP_LDA': '滤波器组共空间模式', 'DFBCSP_FR_LDA': '基于费雪比的判别滤波器组共空间模式'}
    ...
    with open(log_name, 'w') as f:
        ...
        for j, exp_dataset in enumerate(subject_datasets):
            ...
            if model == 'FBCSP_LDA':
                ...
                f.write('select_ratio' + str(select_ratio_fbcsp) + '\n')
                gs = GridSearchCV(FBCSP_estimator_LDA(fs=fs,csp_cpn=2), param_grid=params, cv=4)
                gs.fit(x_train.numpy(), y_train.numpy())
                f.write(str(gs.grid_scores_) + '\n')
                clf_4 = gs.best_estimator_
                joblib.dump(clf_4,'subject'+str(j)+'_'+ 'best_FBCSP_estimator_LDA.m')
            ...
            if model == 'DFBCSP_FR_LDA':
                ...
                gs = GridSearchCV(DFBCSP_FR_estimator_LDA(C3_idx=0, C4_idx=1, fs=fs,csp_cpn=2), param_grid=params, cv=4)
                gs.fit(x_train.numpy(), y_train.numpy())
                f.write(str(gs.grid_scores_) + '\n')
                clf_5 = gs.best_estimator_
                joblib.dump(clf_5,'subject'+str(j)+'_'+'best_DFBCSP_FR_estimator_LDA.m')
...
def compare_CSP_dl_results(datasets,ch,out_file):
    ...
    for j, exp_dataset in enumerate(subject_datasets):
        ...
        p0, cm0 = cm_plot(y=y_test.numpy(), yp=clf_0.predict(x_test.numpy()), classes=datasets.chi_names,
                          sub=str(j + 1), dataset=datasets.name, model='CSP_SVC', ch=ch, return_cm=True)
        p0.plot()
        cm0_all += cm0
        score = clf_0.score(x_test.numpy(), y_test.numpy())
        ...
        p1, cm1 = cm_plot(y=y_test.numpy(), yp=clf_1.predict(x_test.numpy()), classes=datasets.chi_names,
                          sub=str(j + 1), dataset=datasets.name, model='CSSP_LDA', ch=ch, return_cm=True)
        p1.plot()
        cm1_all += cm1
        score = clf_1.score(x_test.numpy(), y_test.numpy())
        ...
        p2, cm2 = cm_plot(y=y_test.numpy(), yp=clf_2.predict(x_test.numpy()), classes=datasets.chi_names,
                          sub=str(j + 1), dataset=datasets.name, model='CSSSP_LDA', ch=ch, return_cm=True)
        p2.plot()
        cm2_all += cm2
        score = clf_2.score(x_test.numpy(), y_test.numpy())
        ...
        p3, cm3 = cm_plot(y=y_test.numpy(), yp=clf_3.predict(x_test.numpy()), classes=datasets.chi_names,
                          sub=str(j + 1), dataset=datasets.name, model='SBCSP_LDA', ch=ch, return_cm=True)
        p3.plot()
        cm3_all += cm3
        score = clf_3.score(x_test.numpy(), y_test.numpy())
        ...
        p4, cm4 = cm_plot(y=y_test.numpy(), yp=clf_4.predict(x_test.numpy()), classes=datasets.chi_names,
                          sub=str(j + 1), dataset=datasets.name, model='FBCSP_LDA', ch=ch, return_cm=True)
        p4.plot()
        cm4_all += cm4
        score = clf_4.score(x_test.numpy(), y_test.numpy())
        ...
        p5, cm5 = cm_plot(y=y_test.numpy(), yp=clf_5.predict(x_test.numpy()), classes=datasets.chi_names,
                          sub=str(j + 1), dataset=datasets.name, model='DFBCSP_FR_LDA', ch=ch, return_cm=True)
        p5.plot()
        cm5_all += cm5
        score = clf_5.score(x_test.numpy(), y_test.numpy())
    ...
    max_res_sub = np.array(res).mean(axis=0).max()
    max_arg_sub = np.array(res).mean(axis=0).argmax()
    out_file.write('在数据集'+str(datasets.name)+'上评估共空间模式模型和深度学习模型的泛化表现,结果显示：表现最佳的模型是'+str(ch_dict[models[max_arg_para]])+'，在'+str(datasets.n_subject)+'名被试中的平均准确率达到'+str('{:.2f}').format(max_res_para+ch)+'。')
    out_file.write('第'+str(max_arg_sub+1)+'名被试表现最佳，平均准确率达到'+str('{:.2f}').format(max_res_sub+ch)+'。\n')
    ...
    plot_acc_bars('results for' + datasets.name, models , res, width=0.8, ch=ch,
                  save=True)

def compare_dl_results(datasets,ch,out_file):
    subject_datasets = []
    for i in range(datasets.n_subject):
        datasets_i = Subject_dataset(database=datasets, subject_id=i)
        subject_datasets.append(datasets_i)
    ...
    for j, exp_dataset in enumerate(subject_datasets):
        ...
        p0, cm0 = cm_plot(y=y_test.numpy(), yp=logits_to_pred(model=model,x=x_test).numpy(), classes=datasets.chi_names,
                          sub=str(j + 1), dataset=datasets.name, model='ShallowConvNet', ch=ch, return_cm=True)
        p0.plot()
        cm0_all += cm0
        ...
        p1, cm1 = cm_plot(y=y_test.numpy(), yp=logits_to_pred(model=model, x=x_test).numpy(), classes=datasets.chi_names,
                          sub=str(j + 1), dataset=datasets.name, model='deepConvNet', ch=ch, return_cm=True)
        p1.plot()
        cm1_all += cm1
        ...
        p2, cm2 = cm_plot(y=y_test.numpy(), yp=logits_to_pred(model=model, x=x_test).numpy(),
                          ...
                          sub=str(j + 1), dataset=datasets.name, model='EEGNet', ch=ch, return_cm=True)
        p2.plot()
        cm2_all += cm2
...
def compare_CSP_results(datasets,ch,out_file):
    ch_dict = {'CSP_SVC': '共空间模式模型', 'CSSP_LDA': '共空间频谱模式模型', 'CSSSP_LDA': '共稀疏频谱空间模式模型', 'SBCSP_LDA': '子带共空间模式模型',
               'FBCSP_LDA': '滤波器组共空间模式模型', 'DFBCSP_FR_LDA': '基于费雪比的判别滤波器组共空间模式模型'}
    subject_datasets = []
    for i in range(datasets.n_subject):
...
def fit_dl_module(log_name,datasets,model,max_epoch,batch_size,ch,out_file,subjects=None):
    subject_datasets = []
    fs = datasets.fs
    if subjects==None:
            ...
            if model == 'FBCSPNet':
                ...
                for params in get_one_para(paras):
                    ...
                    model_fbcspnet.fit(train_dataset=train_data, batch_size=batch_size, max_epoch=max_epoch,
                                       test_dataset=test_data, reg=1, ch=ch,save_name='subject'+str(j)+'_'+'fbcspnet.pth')
            ...
            # "2.deepFBCSPNet"
                ...
                for params in get_one_para(paras):
                    ...
                    model_deep_fbcspnet.fit(train_dataset=train_data, batch_size=batch_size, max_epoch=max_epoch,
                                            test_dataset=test_data, ch=ch,save_name='subject'+str(j)+'_'+'deep_fbcspnet.pth')
            ...
            # "3.EEGNet"
                ...
                for params in get_one_para(paras):
                    ...
                    model_eegnet.fit(train_dataset=train_data, batch_size=batch_size, max_epoch=max_epoch,
                                     test_dataset=test_data, ch=ch,save_name='subject'+str(j)+'_'+'eegnet.pth')
...
def fit_dl_module_const(log_name,datasets,model,max_epoch,batch_size,ch,out_file,subjects=None):
    subject_datasets = []
    fs = datasets.fs
    if subjects==None:
        subjects = range(datasets.n_subject)
    ...
    with open(log_name, 'w') as f:
        ...
        for j, exp_dataset in enumerate(subject_datasets):
            ...
            if model == 'FBCSPNet':
                ...
                paras = {'eps': [1e-3], 'linear_init_std': [0.1]}
...
def fit_rnn_module(log_name,datasets,model,max_epoch,batch_size,ch,out_file):
    subject_datasets = []
    fs = datasets.fs
...
def compare_all_results(datasets,ch,ch_dl,out_file,ratio=0.5,subjects=None):
    ...
    for j, exp_dataset in enumerate(subject_datasets):
        if j in subjects:
            ...
            clf_0 = joblib.load('subject' + str(j) + '_' + 'best_AAR_estimator_LDA.m')
            score = clf_0.score(x_test.numpy(), y_test.numpy())
            cm0 = get_cm(y=y_test.numpy(), yp=clf_0.predict(x_test.numpy()), ch=ch)
            cm0_all += cm0
            AAR_LDA_res.append(score+(1-score)*ch)

            clf_1 = joblib.load('subject' + str(j) + '_' + 'best_AAR_estimator_SVC.m')
            score = clf_1.score(x_test.numpy(), y_test.numpy())
            cm1 = get_cm(y=y_test.numpy(), yp=clf_1.predict(x_test.numpy()), ch=ch)
            cm1_all += cm1
            AAR_SVC_res.append(score+(1-score)*ch)

            clf_2 = joblib.load('subject' + str(j) + '_' + 'best_AAR_estimator_KNN.m')
            score = clf_2.score(x_test.numpy(), y_test.numpy())
            cm2 = get_cm(y=y_test.numpy(), yp=clf_2.predict(x_test.numpy()), ch=ch)
            cm2_all += cm2
            AAR_KNN_res.append(score+(1-score)*ch)


            clf_3 = joblib.load('subject'+str(j)+'_'+'best_CSP_estimator_SVC.m')
            cm3 = get_cm(y=y_test.numpy(), yp=clf_3.predict(x_test.numpy()), ch=ch)
            cm3_all += cm3
            score = clf_3.score(x_test.numpy(), y_test.numpy())
            CSP_SVC_res.append(score+(1-score)*ch)

            clf_4 = joblib.load('subject'+str(j)+'_'+'best_CSSP_estimator_LDA.m')
            cm4 = get_cm(y=y_test.numpy(), yp=clf_4.predict(x_test.numpy()), ch=ch)
            cm4_all += cm4
            score = clf_4.score(x_test.numpy(), y_test.numpy())
            CSSP_LDA_res.append(score+(1-score)*ch)

            clf_5 = joblib.load('subject'+str(j)+'_'+'best_CSSSP_estimator_LDA.m')
            cm5 = get_cm(y=y_test.numpy(), yp=clf_5.predict(x_test.numpy()), ch=ch)
            cm5_all += cm5
            score = clf_5.score(x_test.numpy(), y_test.numpy())
            CSSSP_LDA_res.append(score+(1-score)*ch)

            try:
                clf_6 = joblib.load('subject'+str(j)+'_'+'best_SBSSP_estimator_LDA.m')
                cm6 = get_cm(y=y_test.numpy(), yp=clf_6.predict(x_test.numpy()), ch=ch)
                cm6_all += cm6
                score = clf_6.score(x_test.numpy(), y_test.numpy())
                SBCSP_LDA_res.append(score+(1-score)*ch)
            except:
            ...
            cm7 = get_cm(y=y_test.numpy(), yp=clf_7.predict(x_test.numpy()), ch=ch)
            cm7_all += cm7
            score = clf_7.score(x_test.numpy(), y_test.numpy())
            FBCSP_LDA_res.append(score+(1-score)*ch)

            clf_8 = joblib.load('subject'+str(j)+'_'+'best_DFBCSP_FR_estimator_LDA.m')
            cm8 = get_cm(y=y_test.numpy(), yp=clf_8.predict(x_test.numpy()), ch=ch)
            cm8_all += cm8
            score = clf_8.score(x_test.numpy(), y_test.numpy())
            DFBCSP_FR_LDA_res.append(score+(1-score)*ch)

            if y_test.min()==1:
                y_test -= 1
            model = torch.load('subject'+str(j)+'_'+'fbcspnet.pth')
            test_acc, _ = model.evaluate_train(test_dataset=test_data, batch_size=batch_size)
            FBCSPNet_res.append(float(test_acc.numpy()+(1-test_acc.numpy())*ch_dl))
            try:
                cm9 = get_cm(y=y_test.numpy(), yp=logits_to_pred(model=model,x=x_test).numpy(), ch=ch_dl)
                cm9_all += cm9
            except:
                pass
            ...
            model = torch.load('subject'+str(j)+'_'+'deep_fbcspnet.pth')
            test_acc, _ = model.evaluate_train(test_dataset=test_data, batch_size=batch_size)
            deep_FBCSPNet_res.append(float(test_acc.numpy()+(1-test_acc.numpy())*ch_dl))
            try:
                cm10 = get_cm(y=y_test.numpy(), yp=logits_to_pred(model=model, x=x_test).numpy(), ch=ch_dl)
                cm10_all += cm10
            except:
                pass
            model = torch.load('subject'+str(j)+'_'+'eegnet.pth')
            test_acc, _ = model.evaluate_train(test_dataset=test_data, batch_size=batch_size)
            EEGNet_res.append(float(test_acc.numpy()+(1-test_acc.numpy())*ch_dl))
            try:
                cm11 = get_cm(y=y_test.numpy(), yp=logits_to_pred(model=model,x=x_test).numpy(), ch=ch_dl)
                cm11_all += cm11
            except:
                pass
    ...
    try:
        ...
        pass
    plot_acc_bars_no_anno('results for' + datasets.name, models , res, width=0.8, ch=ch,
                  save=True)

def compare_all_results_just_acc(datasets,ch,out_file,subjects=None,ratio=0.5):
    ch_dict = {'AR_LDA':'基于LDA的自回归模型','AR_SVC':'基于SVM的自回归模型','AR_KNN':'基于kNN的自回归模型','CSP_SVC': '共空间模式模型', 'CSSP_LDA': '共空间频谱模式模型', 'CSSSP_LDA': '共稀疏频谱空间模式模型', 'SBCSP_LDA': '子带共空间模式模型',
               'FBCSP_LDA': '滤波器组共空间模式模型', 'DFBCSP_FR_LDA': '基于费雪比的判别滤波器组共空间模式模型','fbcspnet':'ShallowConvNets','deepfbcspnet':'DeepConvNet','eegnet':'EEGNet'}
    subject_datasets = []
    for i in range(datasets.n_subject):
    ...
    for j, exp_dataset in enumerate(subject_datasets):
        if j in subjects:
                ...
                model = torch.load('subject'+str(j)+'_'+'fbcspnet.pth')
                test_acc, _ = model.evaluate_train(test_dataset=test_data, batch_size=batch_size)
                FBCSPNet_res.append(float(test_acc.numpy()-(1-test_acc.numpy())*ch))


                model = torch.load('subject'+str(j)+'_'+'deep_fbcspnet.pth')
                test_acc, _ = model.evaluate_train(test_dataset=test_data, batch_size=batch_size)
                deep_FBCSPNet_res.append(float(test_acc.numpy()-(1-test_acc.numpy())*ch))


                model = torch.load('subject'+str(j)+'_'+'eegnet.pth')
                test_acc, _ = model.evaluate_train(test_dataset=test_data, batch_size=batch_size)
                EEGNet_res.append(float(test_acc.numpy()-(1-test_acc.numpy())*ch))
    ...
    max_res_sub = np.array(res).mean(axis=0).max()
    max_arg_sub = np.array(res).mean(axis=0).argmax()
    out_file.write('在数据集'+str(datasets.name)+'上评估自回归、共空间模式模型和深度学习模型的泛化表现,结果显示：表现最佳的模型是'+str(ch_dict[models[max_arg_para]])+'，在'+str(datasets.n_subject)+'名被试中的平均准确率达到'+str('{:.2f}').format(max_res_para+ch)+'。')
    out_file.write('第'+str(max_arg_sub+1)+'名被试表现最佳，平均准确率达到'+str('{:.2f}').format(max_res_sub+ch)+'。\n')
    out_file.write('\n')
    for i,model_name in enumerate(models):
        out_file.write(str(model_name)+':')
