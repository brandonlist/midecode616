...
class LiteNet(nn.Module):
    ...
    def __init__(self,in_chans,time_steps,classes,env=None,linear_init_std=0.1,eps=1e-3,pace_1_ratio = 0.05,fs=250,p=0.5):
        #hyperparameters
        self.eps = eps
        self.time_steps = time_steps
        # hyperparameters
        ...
        self.pool_stride = round(self.time_steps*self.pool_stride_ratio)
        self.p = p
        self.conv = nn.Sequential(nn.Conv2d(in_channels=self.in_chans,out_channels=self.n_filters,kernel_size=(self.pace_1,1),bias=False),nn.LeakyReLU())
        self.maxpool = nn.Sequential(nn.MaxPool2d(kernel_size=(self.pool_kernel,1),stride=(self.pool_stride,1)),nn.Dropout2d(p=self.p))

        dummy = torch.Tensor(1,self.in_chans,self.time_steps,1)
        dummy = self.maxpool(self.conv(dummy))
        # input_dim =round (self.n_filters_spat * (self.time_steps-(self.pace_1-1)-(self.pool_kernel-1))/self.pool_stride )
        input_dim = dummy.shape[1]*dummy.shape[2]
    ...
    def fit(self,train_dataset, batch_size, max_epoch, valid_dataset=None, test_dataset=None, log_path=None ,ch=0.3,save_name='LiteNet.pth'):
        """
        fit the model to the dataset
        :param dataset:torch.utils.data.Dateset
        :param model: torch.nn.module
...
# from BCICdatasets import BCIC4_2b
# from BCI_database import Subject_dataset
# from experiment_utils import split_dataset
# dataset = BCIC4_2b()
# dataset.load_data()
# in_chan = dataset.signal_shape()[0]
# time_steps = dataset.signal_shape()[1]
# dataset0 =  Subject_dataset(dataset,0)
# train_data,test_data = split_dataset(dataset0,0.8,0)
#
# model = LiteNet(in_chans=in_chan,time_steps=time_steps,classes=2,linear_init_std=0.01,p=0.5)
# model.fit(train_dataset=train_data,batch_size=128,max_epoch=500,ch=0,test_dataset=test_data)
# x,y = next(iter(DataLoader(train_data,16)))
